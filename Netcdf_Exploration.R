...
#UPDATE: there are now two code chunks to run yearly OR monthly
...
file_names <- list.files("/Users/esaraf/Desktop/NOAA/Jan_12_Plots/Backward_MONT_By_Month/2002", full.names = TRUE)
...
for (f in 1:length(file_names)){
  ...
  month <- as.numeric(unlist(strsplit(file_names[f],"_"))[12])
  print(month)
  
  #----Extract variables from Netcdf-----
  #According to 'print(file)' above, there are 5 variables: trajectory, time, lat, lon, and depth
  id <- ncvar_get(nc_file,'trajectory') #we don't really need to extract this because the dimension of 45 indicates 45 particles. 
  ...
  #----Time-series: proportion of particles south of X degrees-----
  #Adjust latitudes depending on whether simulations are FORWARDS or BACKWARDS
  
  #RUNNINNG BACKWARDS FROM MONTEREY SIMULATION: Count how many particles ended south of 30 degrees (change to "<")
  #RUNNINNG FORWARDS FROM BAJA SIMULATION: Count how many particles ended north of 30 degrees (change to ">")
  ...
  p_all <- dim(id)[2] #number of unique particles in netcdf file
  prop_30 <- (p_30/p_all)
  prop_32 <- (p_32/p_all)
  prop_34 <- (p_34/p_all)
  prop_36 <- (p_36/p_all)
...
file_names <- list.files("~/Dropbox/PRC Particle Tracking/Parcels_output_total/", full.names = TRUE)
